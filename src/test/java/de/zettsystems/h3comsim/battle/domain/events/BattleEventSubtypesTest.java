...
class BattleEventSubtypesTest {
    ...
    private static final StackSnapshot ATTACKER_SNAP =
            new StackSnapshot(Side.ATTACKER, 0, "Pikeman", 5, 10, 0, 5);
    private static final StackSnapshot DEFENDER_SNAP =
            new StackSnapshot(Side.DEFENDER, 0, "Marksman", 5, 10, 14, 5);
    ...
    void move_copies_path_defensively() {
        ...
        BattleEvent.Move move = new BattleEvent.Move(Side.ATTACKER, 0, 0, 5, 1, 5, path);

        path.clear();

        assertThat(move.actor()).isEqualTo(Side.ATTACKER);
        assertThat(move.actorSlot()).isZero();
    ...
    void move_back_copies_path_defensively() {
        ...
        BattleEvent.MoveBack moveBack = new BattleEvent.MoveBack(Side.ATTACKER, 0, 0, 5, path);
    ...
    void wait_event_carries_actor() {
        BattleEvent.Wait waitEvent = new BattleEvent.Wait(Side.DEFENDER, 0);
        assertThat(waitEvent.actor()).isEqualTo(Side.DEFENDER);
        assertThat(waitEvent.actorSlot()).isZero();
    }

    @Test
    void shoot_event_carries_distance_damage_kills_snapshot() {
        BattleEvent.Shoot shoot =
                new BattleEvent.Shoot(Side.ATTACKER, 0, Side.DEFENDER, 0, 8, 47, 3, DEFENDER_SNAP);
    ...
    void melee_event_carries_hexes_moved_damage_kills() {
        BattleEvent.Melee melee =
                new BattleEvent.Melee(Side.ATTACKER, 0, Side.DEFENDER, 0, 3, 62, 4, DEFENDER_SNAP);
    ...
    void retaliation_event_carries_retaliator_target_damage_kills() {
        BattleEvent.Retaliation retaliation =
                new BattleEvent.Retaliation(Side.DEFENDER, 0, Side.ATTACKER, 0, 30, 2, ATTACKER_SNAP);
    ...
    void death_stare_carries_kills_and_snapshot() {
        BattleEvent.DeathStare stare =
                new BattleEvent.DeathStare(Side.ATTACKER, 0, Side.DEFENDER, 0, 2, DEFENDER_SNAP);
    ...
    void thunderbolts_carries_damage_and_snapshot() {
        BattleEvent.Thunderbolts bolt =
                new BattleEvent.Thunderbolts(Side.ATTACKER, 0, Side.DEFENDER, 0, 70, DEFENDER_SNAP);
    ...
    void debuff_skill_events_carry_actor_and_target() {
        BattleEvent.Petrifying petrifying = new BattleEvent.Petrifying(Side.ATTACKER, 0, Side.DEFENDER, 0);
        BattleEvent.Cursing cursing = new BattleEvent.Cursing(Side.DEFENDER, 0, Side.ATTACKER, 0);
        BattleEvent.Poisoning poisoning = new BattleEvent.Poisoning(Side.ATTACKER, 0, Side.DEFENDER, 0);
        BattleEvent.Diseasing diseasing = new BattleEvent.Diseasing(Side.DEFENDER, 0, Side.ATTACKER, 0);
        BattleEvent.Aging aging = new BattleEvent.Aging(Side.ATTACKER, 0, Side.DEFENDER, 0);
    ...
    void fire_shield_carries_reflected_damage_and_attacker_snapshot() {
        BattleEvent.FireShield shield =
                new BattleEvent.FireShield(Side.DEFENDER, 0, Side.ATTACKER, 0, 12, ATTACKER_SNAP);
    ...
    void rebirth_carries_restored_count_and_snapshot() {
        BattleEvent.Rebirth rebirth =
                new BattleEvent.Rebirth(Side.ATTACKER, 0, 1, ATTACKER_SNAP);
    ...
    void battle_end_carries_winner_survivors_turns() {
        ...
        assertThat(end.finalStacks()).containsExactly(ATTACKER_SNAP, DEFENDER_SNAP);
    }
}
